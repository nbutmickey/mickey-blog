<template>
  ...
  <div class="collection-title">
    <h3 style="margin-left: 10px;">{{tagName.tag}}</h3>
  </div>
  <div>
    <div class="post" v-for="(item) in articles">
    <div class="post-header">
      <div class="post-meta">{{time(item.date)}}</div>
      <div class="post-title" style="cursor: pointer"><router-link :to="{name:'articles',params:{id:item.postId}}">{{item.title}}</router-link></div>
    </div>
    </div>
  </div>
</div>
    ...
    export default {
        ...
        methods:{
          getTagArticle:function () {
            this.$http.get('/api/getTagArticle',{params:{tagId:this.$route.params.id}}).then(res=>{
              this.articles=res.data.tagArticle;
